...
// Word Match – ghép TỪ ↔ NGHĨA. UI rộng rãi, dễ bấm; vẽ line nối cặp đúng.
...
function layoutLines() {
  ...
  // hide lines on mobile (1 cột) để đỡ rối
  const isSingleCol = window.matchMedia('(max-width: 767px)').matches;
  if (isSingleCol) return;

  const rect = board.getBoundingClientRect();
  svg.setAttribute('width', String(rect.width));
  svg.setAttribute('height', String(rect.height));

  const { pairs, solved } = state.wordMatchGame;
...
function attachHandlers(container) {
  ...
  const tryMatch = () => {
    ...
    const { pairs, solved } = state.wordMatchGame;
    // vì mỗi pair có id duy nhất và hai cột đều là shuffle của cùng list,
    // match đúng khi id trùng nhau
    if (leftId === rightId) {
      playSound('correct');
      solved.add(leftId);
      ...
      [selectedLeft, selectedRight].forEach(el => {
        ...
        el.classList.add('bg-green-600', 'text-white', 'cursor-default');
        el.disabled = true;
      });
  ...
  leftWrap.querySelectorAll('.wm-left-item').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      // reflow để SVG canh đúng khi font/line-height thay đổi
...
export function startWordMatch(containerId) {
  ...
  container.innerHTML = `
    ...
    <p class="text-gray-500 dark:text-gray-400 mb-4">Chạm chọn 1 từ và 1 nghĩa để ghép. Đúng sẽ tô xanh và vẽ đường nối.</p>
    ...
    <div id="word-match-board"
      ...
      <!-- Lines layer (ẩn trên mobile để đỡ rối) -->
